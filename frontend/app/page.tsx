...
import Image from 'next/image'; // [필수] Image 컴포넌트 임포트 확인
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen text-white p-4 font-sans">
      
      {/* [수정됨] 히어로 섹션: 이미지 배너로 교체 */}
      <div className="w-full mb-8 relative rounded-3xl overflow-hidden shadow-2xl border border-slate-700 group">
        {/* 이미지 비율 유지 (모바일 2:1, 데스크탑 3:1) */}
        <div className="relative w-full aspect-[2/1] md:aspect-[3/1]">
           {/* public/hero-banner.png 파일이 필요합니다. 없을 경우 대비해 대체 텍스트 표시 */}
           <Image 
              src="/hero-banner.jpg" 
              ...
              priority
           />
           {/* 이미지가 없을 때를 대비한 폴백 배경 (이미지 넣으면 가려짐) */}
           <div className="absolute inset-0 bg-slate-800 -z-10 flex items-center justify-center">
              <span className="text-slate-500">배너 이미지를 등록해주세요</span>
           </div>
           
           {/* (선택사항) 이미지 위에 은은한 그라데이션 오버레이를 씌워 텍스트 가독성을 높이거나 분위기를 줌 */}
           <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent"></div>
        </div>
      </div>

      {/* 탭 메뉴 */}
      <div className="flex w-full max-w-md bg-slate-800 p-1 rounded-2xl mb-8 shadow-lg z-10 border border-slate-700">
        {[
      ...
      <div className="w-full max-w-2xl space-y-6 relative z-0 min-h-[500px]">
        {/* === 탭 1: 로또 === */}
        {activeTab === 'lotto' && (
          <div className="animate-fade-in">
            <div className="p-6 border border-slate-700 rounded-3xl bg-slate-800/50 backdrop-blur-sm shadow-xl mb-6">
              {lotto ? (
            ...
            <div className="p-1 relative mb-6">
            ...
            {prediction.length > 0 && (
              <div className="space-y-4 animate-fade-in-up mb-12">
            ...
            {/* 명예의 전당 (한글화 및 회차 정보 복구 완료) */}
            <div className="mt-12 p-6 bg-slate-800/80 rounded-3xl border border-slate-700 backdrop-blur-md">
            ...
            <div className="flex gap-4 mt-8 pt-6 border-t border-slate-700">
               <Link href="/map" className="flex-1">
                 <div className="bg-slate-800 border border-slate-600 rounded-2xl p-6 flex flex-col items-center gap-2 hover:bg-slate-700 transition-colors shadow-lg cursor-pointer h-32 justify-center group">
                    ...
                    <span className="text-xs text-slate-500">주변 판매점 찾기</span>
                 </div>
               </Link>
               <Link href="/ranking" className="flex-1">
                 <div className="bg-slate-800 border border-slate-600 rounded-2xl p-6 flex flex-col items-center gap-2 hover:bg-slate-700 transition-colors shadow-lg cursor-pointer h-32 justify-center group">
                    ...
                    <span className="text-xs text-slate-500">역대 최다 배출 순위</span>
                 </div>
               </Link>
            </div>
          </div>
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-900 border border-slate-600 rounded-xl p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none text-sm font-mono [color-scheme:dark]"
                      />
                    </div>
                    <div className="flex-1">
