...
function MapContent() {
  ...
  useEffect(() => {
    if (map && stores.length > 0) {
      stores.forEach(store => {
        ...
        const iwContent = `
          <div style="padding:10px; color:#333; font-size:12px; min-width:180px; border-radius:8px; background:white;">
            ...
            <div style="font-size:11px;">1등: <span style="color:red; font-weight:bold;">${store.first_count}회</span> / 2등: <span style="color:blue;">${store.second_count}회</span></div>
          </div>
        `;
      ...
      if (focusLat && focusLng && focusName) {
        ...
        map.setLevel(3); 
        const iwContent = `<div style="padding:5px; color:black; font-weight:bold;">📍 ${focusName}</div>`;
  ...
  const handleFindMe = () => {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      map.setCenter(loc);
      map.setLevel(5);
      new window.kakao.maps.Marker({ position: loc, map: map, title: "내 위치" });
  ...
  return (
    <>
      ...
      {/* [수정] top-[70px] -> top-24 (96px) : 확실하게 내림 */}
      ...
      <button 
        ...
        disabled={isFinding}
        className={`absolute bottom-8 right-4 z-20 bg-blue-600 text-white p-3 rounded-full shadow-xl font-bold text-xs font-mono border border-blue-400 transition-all ${isFinding ? 'opacity-80 cursor-wait' : 'hover:bg-blue-500'}`}
      >
        {isFinding ? "탐색 중..." : "📍 내 위치"}
      </button>

      <div id="map" className="w-full h-[100dvh] bg-slate-900 relative">
        {!isMapLoaded && (
